...
namespace emdl
{
	...
	public:
		...
		constexpr ArrayView(U (&data)[size]) noexcept
			: m_data{data}
			, m_size{size}
		{
		}
		...
		constexpr ArrayView(ArrayView<U> array) noexcept
			: m_data{array}
			, m_size{array.size()}
		{
		}
	...
	public:
		...
		constexpr ArrayView(std::nullptr_t) noexcept {}
		constexpr ArrayView() noexcept {}
		constexpr ArrayView(const void* data, std::size_t size) noexcept
			: m_data(data)
			, m_size(size)
		{
		}
		...
		constexpr ArrayView(T (&data)[size]) noexcept
			: m_data(data)
			, m_size(size * sizeof(T))
		{
		}
		...
		template <class T>
		constexpr ArrayView(const ArrayView<T>& array) noexcept
			: m_data(array)
			, m_size(array.size() * sizeof(T))
		{
		}
