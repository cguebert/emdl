...
namespace
{
	/// Remove the trailing zeros of the mantissa.
	...
	/**
	 * @brief Round the integer contained in (buffer[0], buffer[n]) (end included) to the nearest factor of 10.
...
namespace emdl
{
	void writeDS(double f, char* buffer, int size)
	{
		...
		if (line[0] == '-')
		{
			// Happens in the case of -0, other negative numbers have been already handled
			f = -f;
			size -= 1;
			*buffer = '-';
			++buffer;
